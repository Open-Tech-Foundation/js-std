...
const pkgDir = 'packages/std';
const srcDir = path.join(pkgDir, 'src');
const docsDir = path.join(pkgDir, 'docs');
const pkgReadmePath = path.join(pkgDir, 'README.md');
...
function updateReadme(readmePath, linkPrefix) {
  ...
  Object.keys(indexData).sort().forEach(cat => {
    ...
    readme = readme.replace(regex, (match, p1, p2) => {
      ...
      let newList = `${header}\n\n`;
      indexData[cat].sort().forEach(name => {
        newList += `- [${name}](${linkPrefix}${cat}/${name}.md)\n`;
      });
      return newList + '\n' + p2;
    });
  });
...
// Update both READMEs
updateReadme(pkgReadmePath, './docs/');
updateReadme(rootReadmePath, './packages/std/docs/');

console.log('Docs generated and READMEs updated successfully.');
