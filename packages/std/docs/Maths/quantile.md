...
fraction between the neighbouring values applied. This is the default of R's
`quantile`, NumPy's `percentile` and Excel's `PERCENTILE.INC` — the method
usually meant by "the 95th percentile" — but it is one of nine in common use,
and a figure produced by another will differ on small samples.
