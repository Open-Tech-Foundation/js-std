...
- **seedTime** `number` _(default: `Date.now()`)_ — The timestamp to encode, in milliseconds.

## Returns

`string` — A 26-character ULID.

## Throws

- `RangeError` — If `seedTime` is not an integer within the 48-bit range.
