...
Flattens a nested object into a single level, keyed by the path to each value.
...
array has no leaves to stand for it and so is kept as a value, since dropping it
would lose a key that was present.

A key already containing a `.` or a `[` cannot be told apart from the path built
around it: `{ 'a.b': 1 }` flattens to the same `'a.b'` that `{ a: { b: 1 } }`
does. That is a property of the format rather than of this function —
`unflattenObject` reads both back as the nested form.
