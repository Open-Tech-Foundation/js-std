...
- **options** `{ includePrerelease?: boolean }` _(optional)_ — Set `includePrerelease` to match pre-releases anywhere.

## Returns

`boolean` — `true` if the version satisfies the range.

## Throws

- `TypeError` — If the version or the range cannot be parsed.
