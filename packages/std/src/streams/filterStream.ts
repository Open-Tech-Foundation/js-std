 ...
 */
export default function filterStream<T>(
  ...
  return new ReadableStream<T>({
    async pull(controller) {
      ...
      // Keep reading until something passes, so one requested chunk yields one
      // chunk however many were rejected on the way.
      while (true) {
