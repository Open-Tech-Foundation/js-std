...
describe('IterAsync Helpers', () => {
  ...
  test('filterIterAsync', async () => {
    ...
    const asyncEvens = filterIterAsync(asyncGen([1, 2, 3, 4]), async (x) => x % 2 === 0);
  ...
  test('flatMapIterAsync', async () => {
    ...
    const asyncFlattened = flatMapIterAsync(asyncGen([1, 2]), async (x) => asyncGen([x, x * 10]));
    expect(await toArrayIterAsync(asyncFlattened)).toEqual([1, 10, 2, 20]);
  });

  test('reduceIterAsync', async () => {
    const sum = await reduceIterAsync(asyncGen([1, 2, 3]), (acc, x) => acc + x, 0);
    expect(sum).toBe(6);

    const asyncSum = await reduceIterAsync(asyncGen([1, 2, 3]), async (acc, x) => acc + x, 10);
    expect(asyncSum).toBe(16);
  });

  test('eachIterAsync', async () => {
    const result: number[] = [];
    await eachIterAsync(asyncGen([1, 2]), (x) => { result.push(x); });
    expect(result).toEqual([1, 2]);

    const asyncResult: number[] = [];
    await eachIterAsync(asyncGen([1, 2]), async (x) => { asyncResult.push(x); });
  ...
  test('someIterAsync', async () => {
    ...
    expect(await someIterAsync(asyncGen([1, 2, 3]), async (x) => x === 2)).toBe(true);
  });
  ...
  test('everyIterAsync', async () => {
    ...
    expect(await everyIterAsync(asyncGen([1, 2, 3]), async (x) => x < 5)).toBe(true);
  });

  test('findIterAsync', async () => {
    expect(await findIterAsync(asyncGen([1, 2, 3]), (x) => x > 1)).toBe(2);
    expect(await findIterAsync(asyncGen([1, 2, 3]), (x) => x > 5)).toBe(undefined);
    expect(await findIterAsync(asyncGen([1, 2, 3]), async (x) => x === 3)).toBe(3);
  });

  describe('Edge Cases', () => {
    test('empty iterators', async () => {
        expect(await toArrayIterAsync(asyncGen([]))).toEqual([]);
        expect(await reduceIterAsync(asyncGen([]), (acc, x) => acc + x, 0)).toBe(0);
    ...
    test('errors in callbacks', async () => {
        try {
            await mapIterAsync(asyncGen([1]), () => { throw new Error('Callback error'); }).next();
        } catch (e) {
  ...
  describe('New Iter Utilities', () => {
    test('fromIterAsync', async () => {
      ...
      expect(await toArrayIterAsync(fromIterAsync({ next: () => ({ value: 1, done: true }) }))).toEqual([]);
    });
    ...
    test('takeIterAsync', async () => {
      ...
      expect(await toArrayIterAsync(takeIterAsync(asyncGen([1]), 5))).toEqual([1]);
      expect(await toArrayIterAsync(takeIterAsync(asyncGen([1, 2]), 0))).toEqual([]);
    ...
    test('dropIterAsync', async () => {
      ...
      expect(await toArrayIterAsync(dropIterAsync(asyncGen([1, 2]), 0))).toEqual([1, 2]);
    ...
    test('findLastIterAsync', async () => {
      expect(await findLastIterAsync(asyncGen([1, 2, 3, 2]), x => x === 2)).toBe(2);
      expect(await findLastIterAsync(asyncGen([1, 2, 3]), x => x > 5)).toBe(undefined);
    });

    test('findIndexIterAsync', async () => {
      expect(await findIndexIterAsync(asyncGen([1, 2, 3]), x => x === 2)).toBe(1);
      expect(await findIndexIterAsync(asyncGen([1, 2, 3]), x => x > 5)).toBe(-1);
    });

    test('findLastIndexIterAsync', async () => {
      expect(await findLastIndexIterAsync(asyncGen([1, 2, 3, 2]), x => x === 2)).toBe(3);
      expect(await findLastIndexIterAsync(asyncGen([1, 2, 3]), x => x > 5)).toBe(-1);
    });
  });
