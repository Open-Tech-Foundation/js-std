...
- Try it now prints the result of each annotated expression, including examples whose expected result is written on the following line; explicit console calls remain single-output.
...
- Regenerated the Color API reference so its parameter lists match the object-based conversion APIs.
...
- **Breaking.** The color derivatives that take more than a single color, or a color plus an adjustment, now take a single object naming their arguments, matching `color`. `format` becomes `to`, and the value or colours become named keys: `colorAlpha(x, 0.5, 'rgba')` becomes `colorAlpha({ value: x, amount: 0.5, to: 'rgba' })`, `colorMix(a, b, 0.5, 'rgb')` becomes `colorMix({ color1: a, color2: b, weight: 0.5, to: 'rgb' })`, and `colorContrast(a, b)` becomes `colorContrast({ color1: a, color2: b })`. Only the functions that take a bare color and nothing else — `colorIsDark`, `colorIsLight` and `colorLuminance` — keep their positional signatures, since an object wrapper would add ceremony without naming anything. The full set converted is `colorAlpha`, `colorLighten`, `colorDarken`, `colorSaturate`, `colorDesaturate`, `colorRotateHue`, `colorGrayscale`, `colorInvert`, `colorMix`, `colorContrast`, `colorIsReadable` and `colorWCAGLevel`.
...
- The seven color adjusters — `colorLighten`, `colorDarken`, `colorSaturate`, `colorDesaturate`, `colorRotateHue`, `colorAlpha` and `colorMix` — accepted a non-finite amount and returned something that was not a color. `color` has always refused an unreadable *color*, but the *number* beside it went unchecked, and each function failed differently: the lightness, saturation and mix paths produced the literal string `#NaNNaNNaN`, a hue rotation of `NaN` turned `#3366cc` into `#333333` — a real color, but not the one passed in — and an alpha of `NaN` was dropped, leaving the color silently opaque. `colorMix` did it for the infinities as well, not only `NaN`. A colour string that is not a colour fails wherever it is finally used rather than where it was made, so all seven now throw a `RangeError` naming the parameter. Finite values that are merely out of range still clamp as before.
- `memoizeRun` retained every symbol it ever saw for the life of the process. The identities behind the cache key lived in a module-level strong `Map` keyed by symbol, shared by every memoized function and emptied by nothing — not `clear()`, not eviction, not the cache going out of scope — so the bound placed on the cache in 0.18.0 did not bound this: 200,000 symbol-keyed calls against a cache of two entries held about 26 MB after `clear()`, and now hold 0.2 MB. The record is a `WeakMap`, and registered symbols skip it entirely: `Symbol.for('a')` is interned by its key, so the key identifies it without anything being stored, which also keeps them out of a `WeakMap` that cannot hold them.
- `isEql` threw on a circular array, `Map` or `Set` instead of comparing it. Only plain objects were recorded as the walk descended, so a container that held itself was never recognised as a cycle and the comparison ran until the depth cap raised a `RangeError` — a self-referencing array is the ordinary shape of a parsed graph or a linked structure, not a hostile input. Every container is recorded now. The record is also of *pairs* rather than of each side separately: two sets, one per graph, called it a cycle as soon as each value had been seen anywhere in its own side, which is true of structures that merely share a node and are not equal at all, so that false positive is gone with it. The docs now state that `Set` values are compared in iteration order, as `Map` entries already were.
...
- `retryRun` validated its retry count with `Number.isInteger`, which is true of `1e308`, so a count taken from configuration passed and then drove a loop with no end. With no delay that loop holds no timer either, only awaited promises, so it starved the event loop rather than merely running long — a timer set before the call was still unfired twenty seconds later, and nothing else in the process ran. Retries are capped at `MAX_RETRIES` (1000).
- The type guards for `Map`, `Set`, `WeakMap`, `WeakSet`, `Date`, `RegExp`, `ArrayBuffer` and `DataView` read the object's `Object.prototype.toString` tag, which any object can set for itself through `Symbol.toStringTag`. A plain object tagged `Map` passed `isMap`, and `clone` and `isEql` both threw `TypeError` on it when they went to iterate it. They probe the internal slot now, which cannot be faked and still works across realms — `instanceof` does not. This is not reachable from JSON, symbols not being JSON keys, so it is a robustness fix rather than a boundary.

- The recursive walkers — `merge`, `mergeAll`, `clone`, `isEql` and `deepFreeze` — now stop at `MAX_DEPTH` (512) with a `RangeError` naming the function, rather than running until the stack overflows. `JSON.parse` accepts nesting far deeper than a recursive walk over the result can follow, so a 20,000-deep object from untrusted input overflowed. It was a `RangeError` either way, but the depth it happened at depended on the runtime and on how much stack the caller had already used, so the same input could pass in one place and fail in another. 512 is far past any structure meant for a human or a schema.
