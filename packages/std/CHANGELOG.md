...
- `DateTime` uses the TC39 Temporal API where the runtime provides it and `Intl` everywhere else, behind a single internal interface. Temporal has landed in browsers — Chrome and Edge 144+, Firefox 139+ — and ships on Deno and ES-Runtime, but it is still absent on Bun, LLRT and Safari, and on Node.js needs `--harmony-temporal`. A Temporal-only implementation would therefore not have run on most server-side targets, including the one this package is developed and tested on. The two backends are held to identical behaviour by a differential test that exercises both across a table of zones, DST transitions, instants and durations wherever Temporal exists.
