import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "const a = { a: { b: [1] } };\nconst b = { a: { b: [2] } };\nconsole.log(mergeAll(a, b)); //=> { a: { b: [1, 2] }";
import { Callout } from "@opentf/web-docs";
export const metadata = {
  title: "mergeAll — Object",
  description: "It deeply merges objects and concatenates the arrays if any present."
};


It deeply merges objects and concatenates the arrays if any present.

## Syntax

```ts
import { mergeAll } from '@opentf/std';
mergeAll(obj1, ...objN);
```

<Callout type="info" emoji="✅">
Immutable: This does not mutate the given arrays or objects.
</Callout>

<Callout type="info">
  Both string-keyed and symbol-keyed object properties participate in the deep merge. Arrays at matching keys are concatenated.
</Callout>

## Examples

```ts
const a = { a: { b: [1] } };
const b = { a: { b: [2] } };
mergeAll(a, b); //=> { a: { b: [1, 2] }
```

## Try it

<TryIt code={tryItCode} />
