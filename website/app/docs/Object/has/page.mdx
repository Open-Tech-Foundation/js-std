import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(has({ a: 1 }, 'a')); //=> true\n\nconsole.log(has({ a: undefined }, 'a')); //=> true\n\nconsole.log(has({ a: null }, 'a')); //=> true\n\nconsole.log(has({ a: { b: [1, 2, 3, { c: 5 }] } }, 'a.b[3].c')); //=> true\n\nconsole.log(has({ a: { b: [1, 2, 3, { c: 5 }] } }, 'a.b[5].c')); //=> false\n\nconsole.log(has({ a: { 'b.c': 1 } }, ['a', 'b.c'])); //=> true";
import { Callout } from "@opentf/web-docs";
export const metadata = {
  title: "has — Object",
  description: "Checks if a specific path exists in an object."
};


🎯 Checks if a specific path exists in an object.

<Callout type="info">
  This function uses `Object.hasOwn()` internally, so it correctly detects properties even if their value is `undefined` or `null`.
</Callout>

## Syntax

```ts
import { has } from '@opentf/std';
has(obj: object, path: string | unknown[]): boolean
```

## Parameters

- `obj`: The object to inspect.
- `path`: The path to check.

## Returns

`true` if the path exists, `false` otherwise.

## Examples

```ts
has({ a: 1 }, 'a') //=> true

has({ a: undefined }, 'a') //=> true

has({ a: null }, 'a') //=> true

has({ a: { b: [1, 2, 3, { c: 5 }] } }, 'a.b[3].c') //=> true

has({ a: { b: [1, 2, 3, { c: 5 }] } }, 'a.b[5].c') //=> false

has({ a: { 'b.c': 1 } }, ['a', 'b.c']) //=> true
```

## Related

- [get](/docs/Object/get)
- [set](/docs/Object/set)
- [unset](/docs/Object/unset)

<TryIt code={tryItCode} />
