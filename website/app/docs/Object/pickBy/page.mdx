import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "const object = { a: 1, b: 'abc', c: 3 };\n\nconsole.log(pickBy(object, (v) => typeof v === 'number'));\n//=> { a: 1, c: 3 }";
import { Callout } from "@opentf/web-docs";
export const metadata = {
  title: "pickBy — Object",
  description: "Creates an object composed of the object properties that satisfy the predicate."
};

🎯 Creates an object composed of the object properties that satisfy the predicate.

## Syntax

```ts
import { pickBy } from '@opentf/std';
pickBy<T extends object>(
  obj: T,
  predicate: (value: any, key: string | symbol) => boolean
): Partial<T>;
```

## Parameters

- `obj`: The source object.
- `predicate`: The function invoked per iteration. Enumerable symbol keys are included.

When the source object uses `Object.create(null)`, the returned object preserves that null prototype.

## Returns

A new object with picked properties.

<Callout type="warning" emoji="🛡️">
  `__proto__`, `constructor` and `prototype` are skipped. This is often used as an
  allow-list over untrusted input, where copying one of those keys would hand the
  caller a result whose prototype the input chose.
</Callout>

## Examples

```ts
const object = { a: 1, b: 'abc', c: 3 };

pickBy(object, (v) => typeof v === 'number')
//=> { a: 1, c: 3 }
```

## Related

- [pick](/docs/Object/pick)
- [omitBy](/docs/Object/omitBy)

## Try it

<TryIt code={tryItCode} />
