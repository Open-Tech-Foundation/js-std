import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "const a = { a: { b: 1 }, d: 3 };\nconst b = { a: { c: 2 }, e: 5 };\nconsole.log(merge(a, b)); //=> { a: { b: 1, c: 2 }, d: 3, e: 5 }\n\nconst a = { a: [1, 2] };\nconst b = { a: [3, 4, 5] };\nconst c = { a: [7, 8] };\nconsole.log(merge(a, b, c)); //=> { a: [7, 8, 5] }";
import { Callout } from "@opentf/web-docs";
export const metadata = {
  title: "merge — Object",
  description: "It deeply merges objects or arrays."
};


It deeply merges objects or arrays.

## Syntax

```ts
import { merge } from '@opentf/std';
merge(obj1, ...objN);
```

<Callout type="info" emoji="✅">
Immutable: This does not mutate the given arrays or objects.
</Callout>

## Examples

```ts
const a = { a: { b: 1 }, d: 3 };
const b = { a: { c: 2 }, e: 5 };
merge(a, b); //=> { a: { b: 1, c: 2 }, d: 3, e: 5 }

const a = { a: [1, 2] };
const b = { a: [3, 4, 5] };
const c = { a: [7, 8] };
merge(a, b, c); //=> { a: [7, 8, 5] }
```

## Try it

<TryIt code={tryItCode} />
