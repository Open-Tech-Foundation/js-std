import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(colorLuminance('white')); //=> 1\nconsole.log(colorLuminance('black')); //=> 0\nconsole.log(colorLuminance('red')); //=> 0.2126";
export const metadata = {
  title: "colorLuminance — Colors",
  description: "Calculates the relative luminance of a color."
};

💡 Calculates the relative luminance of a color.

## Syntax

```ts
import { colorLuminance } from '@opentf/std';
colorLuminance(input: any): number
```

Returns a value between `0` (darkest black) and `1` (brightest white).

## Examples

```ts
colorLuminance('white'); //=> 1
colorLuminance('black'); //=> 0
colorLuminance('red'); //=> 0.2126
```

<TryIt code={tryItCode} />
