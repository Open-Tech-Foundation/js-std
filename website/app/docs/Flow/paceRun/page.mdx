...
export const tryItCode = "const run = paceRun((val) => console.log(val), 500);\n\nrun('a'); // Logs 'a' immediately (leading edge)\nrun('b'); // Queued for trailing edge (default trailing: true)";
export const metadata = {
  title: "paceRun — FlowControl",
  description: "Creates a throttled function that only invokes func at most once per every interval milliseconds."
};
