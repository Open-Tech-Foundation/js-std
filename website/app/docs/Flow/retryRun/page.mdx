...
export const tryItCode = "const result = await retryRun(() => fetchData(), {\n  retries: 3,\n  delay: 1000,\n  backoff: 'exponential'\n});";
export const metadata = {
  title: "retryRun — FlowControl",
  description: "Retries an asynchronous function until it succeeds or reaches the maximum number of attempts."
};
...
`retries` must be a non-negative integer, `delay` must be a non-negative finite number, and `backoff` must be either `'fixed'` or `'exponential'`.
