import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(await toArrayIterAsync(toAsyncIter([1, 2, 3]))); //=> [1, 2, 3]";
export const metadata = {
  title: "toAsyncIter — Iter",
  description: "Converts a sync iterable to an async iterable."
};

🔄 Converts a sync iterable to an async iterable.

## Syntax

```ts
import { toAsyncIter } from '@opentf/std';
toAsyncIter<T>(iter: Iterable<T>): AsyncIterableIterator<T>
```

## Parameters

- `iter`: The sync iterable to convert.

## Returns

A new async iterable iterator.

## Examples

```ts
await toArrayIterAsync(toAsyncIter([1, 2, 3])) //=> [1, 2, 3]
```

## Related

- [fromIterAsync](/docs/Iter/fromIterAsync)
- [mapIterAsync](/docs/Iter/mapIterAsync)

## Try it

<TryIt code={tryItCode} />
