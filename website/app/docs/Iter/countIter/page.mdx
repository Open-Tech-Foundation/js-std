import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(countIter((function*() { yield 1; yield 2; })())); //=> 2";
export const metadata = {
  title: "countIter — Iter",
  description: "Returns the total count of items in an iterable."
};

🧩 Returns the total count of items in an iterable.

## Syntax

```ts
import { countIter } from '@opentf/std';
countIter(iter: Iterable<unknown>): number
```

## Parameters

- `iter`: The source iterable.

## Returns

The total count of items.

## Examples

```ts
countIter((function*() { yield 1; yield 2; })()) //=> 2
```

## Related

- [countIterAsync](/docs/Iter/countIterAsync)

## Try it

<TryIt code={tryItCode} />
