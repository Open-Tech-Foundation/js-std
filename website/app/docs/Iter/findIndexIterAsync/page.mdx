...
export const tryItCode = "async function* gen() { yield 1; yield 2; }\nconsole.log(await findIndexIterAsync(gen(), x => x === 2)); //=> 1";
export const metadata = {
  title: "findIndexIterAsync — Iter",
  description: "Finds the index of the first item in an async iterator that matches a predicate."
};
