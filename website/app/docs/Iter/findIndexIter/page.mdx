import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(findIndexIter([1, 2, 3], (x) => x === 2)); //=> 1";
export const metadata = {
  title: "findIndexIter — Iter",
  description: "Finds the index of the first item that matches a predicate."
};

🧩 Finds the index of the first item that matches a predicate.

## Syntax

```ts
import { findIndexIter } from '@opentf/std';
findIndexIter<T>(iter: Iterable<T>, fn: (val: T) => boolean): number
```

## Parameters

- `iter`: The source iterable.
- `fn`: The predicate function.

## Returns

The index of the first matching item, or -1.

## Examples

```ts
findIndexIter([1, 2, 3], (x) => x === 2) //=> 1
```

## Related

- [findIndexIterAsync](/docs/Iter/findIndexIterAsync)

## Try it

<TryIt code={tryItCode} />
