import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(findLastIndexIter([1, 2, 3, 2], (x) => x === 2)); //=> 3";
export const metadata = {
  title: "findLastIndexIter — Iter",
  description: "Finds the index of the last item that matches a predicate."
};

🧩 Finds the index of the last item that matches a predicate.

## Syntax

```ts
import { findLastIndexIter } from '@opentf/std';
findLastIndexIter<T>(iter: Iterable<T>, fn: (val: T) => boolean): number
```

## Parameters

- `iter`: The source iterable.
- `fn`: The predicate function.

## Returns

The index of the last matching item, or -1.

## Examples

```ts
findLastIndexIter([1, 2, 3, 2], (x) => x === 2) //=> 3
```

## Related

- [findLastIndexIterAsync](/docs/Iter/findLastIndexIterAsync)

## Try it

<TryIt code={tryItCode} />
