import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log([...dropWhileIter([1, 2, 3, 4], (x) => x < 3)]); //=> [3, 4]";
export const metadata = {
  title: "dropWhileIter — Iter",
  description: "Skips items while a predicate is true, then yields the rest."
};

🧩 Skips items while a predicate is true, then yields the rest.

## Syntax

```ts
import { dropWhileIter } from '@opentf/std';
dropWhileIter<T>(iter: Iterable<T>, fn: (val: T) => boolean): IterableIterator<T>
```

## Parameters

- `iter`: The source iterable.
- `fn`: The predicate function.

## Returns

A new iterable iterator.

## Examples

```ts
[...dropWhileIter([1, 2, 3, 4], (x) => x < 3)] //=> [3, 4]
```

## Related

- [dropWhileIterAsync](/docs/Iter/dropWhileIterAsync)

<TryIt code={tryItCode} />
