import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "async function* gen() { yield 1; yield 2; yield 3; }\nconsole.log(await toArrayIterAsync(dropWhileIterAsync(gen(), x => x < 2))); //=> [2, 3]";
export const metadata = {
  title: "dropWhileIterAsync — Iter",
  description: "Skips items from an async iterable while a predicate is true, then yields the rest."
};

🧩 Skips items while a predicate is true, then yields the rest.

## Syntax

```ts
import { dropWhileIterAsync } from '@opentf/std';
dropWhileIterAsync<T>(iter: AsyncIterable<T>, fn: (val: T) => boolean | Promise<boolean>): AsyncIterableIterator<T>
```

## Parameters

- `iter`: The source async iterable.
- `fn`: The predicate function.

## Returns

A new async iterable iterator.

## Examples

```ts
async function* gen() { yield 1; yield 2; yield 3; }
await toArrayIterAsync(dropWhileIterAsync(gen(), x => x < 2)) //=> [2, 3]
```

## Related

- [dropWhileIter](/docs/Iter/dropWhileIter)

<TryIt code={tryItCode} />
