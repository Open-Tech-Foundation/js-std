import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(randomBytes(16)); //=> Uint8Array(16) [ ... ]";
export const metadata = {
  title: "randomBytes — Crypto",
  description: "Generates an array of cryptographically secure random bytes."
};

🔢 Generates an array of cryptographically secure random bytes.

Uses the standard Web Crypto API (`globalThis.crypto`).

## Syntax

```ts
import { randomBytes } from '@opentf/std';
randomBytes(size: number): Uint8Array;
```

## Parameters

- `size`: The number of bytes to generate.

`size` must be a non-negative integer.

## Returns

A `Uint8Array` containing the random bytes.

## Examples

```ts
randomBytes(16) //=> Uint8Array(16) [ ... ]
```

## Related

- [randomInt](/docs/Crypto/randomInt)

<TryIt code={tryItCode} />
