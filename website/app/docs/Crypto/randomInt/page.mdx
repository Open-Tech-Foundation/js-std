import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(randomInt(0, 100)); //=> 42\nconsole.log(randomInt(1, 10)); //=> 7";
export const metadata = {
  title: "randomInt — Crypto",
  description: "Generates a cryptographically secure random integer between min (inclusive) and max (inclusive)."
};

🔢 Generates a cryptographically secure random integer between min (inclusive) and max (inclusive).

Uses the standard Web Crypto API (`globalThis.crypto`).

## Syntax

```ts
import { randomInt } from '@opentf/std';
randomInt(min: number, max: number): number;
```

## Parameters

- `min`: The minimum value (inclusive).
- `max`: The maximum value (inclusive).

Both bounds must be integers.

## Returns

A random integer.

## Examples

```ts
randomInt(0, 100) //=> 42
randomInt(1, 10) //=> 7
```

## Related

- [randomId](/docs/Crypto/randomId)

<TryIt code={tryItCode} />
