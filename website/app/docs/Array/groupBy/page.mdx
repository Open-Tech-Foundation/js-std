import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(groupBy([])); //=> {}\n\nconsole.log(groupBy([\"a\"])); //=> { undefined: [\"a\"] }\n\nconst products = [\n  { name: \"apples\", category: \"fruits\" },\n  { name: \"oranges\", category: \"fruits\" },\n  { name: \"potatoes\", category: \"vegetables\" },\n];\nconsole.log(groupBy(products, \"category\"));\n//=> {\n//   fruits: [\n//     { name: \"apples\", category: \"fruits\" },\n//     { name: \"oranges\", category: \"fruits\" },\n//   ],\n//   vegetables: [{ name: \"potatoes\", category: \"vegetables\" }],\n// };\n\nconsole.log(groupBy([1, 2, 3, 4, 5, 6, 7, 8, 9], (v) => (v % 2 === 0 ? \"Even\" : \"Odd\")));\n//=> {\n//  Even: [2, 4, 6, 8],\n//  Odd: [1, 3, 5, 7, 9]\n// }\n\nconsole.log(groupBy([\"one\", \"two\", \"three\"], \"length\"));\n//=> {\n//   3: [\"one\", \"two\"],\n//   5: [\"three\"],\n// }";
import { Callout } from "@opentf/web-docs";
export const metadata = {
  title: "groupBy — Array",
  description: "Groups elements of an array based on a specified key or iteratee function."
};

📂 Groups elements of an array based on a specified key or iteratee function.

## Syntax

```ts
import { groupBy } from '@opentf/std';
groupBy<T>(
  arr: T[],
  key: ((val: T, index: number, arr: T[]) => string) | string
): Record<string, T[]>
```

## Parameters

- `arr`: The source array.
- `key`: A property name or a function that returns a string to use as the group key.

## Returns

An object where keys are the result of the iteratee and values are arrays of elements that produced that key.

<Callout type="warning" emoji="🛡️">
  Keys of `__proto__`, `constructor` and `prototype` are refused and their elements
  dropped, as writing one would reach the prototype of the result.
</Callout>

## Examples

```ts
groupBy([]) //=> {}

groupBy(["a"]) //=> { undefined: ["a"] }

const products = [
  { name: "apples", category: "fruits" },
  { name: "oranges", category: "fruits" },
  { name: "potatoes", category: "vegetables" },
];
groupBy(products, "category")
//=> {
//   fruits: [
//     { name: "apples", category: "fruits" },
//     { name: "oranges", category: "fruits" },
//   ],
//   vegetables: [{ name: "potatoes", category: "vegetables" }],
// };

groupBy([1, 2, 3, 4, 5, 6, 7, 8, 9], (v) => (v % 2 === 0 ? "Even" : "Odd"))
//=> {
//  Even: [2, 4, 6, 8],
//  Odd: [1, 3, 5, 7, 9]
// }

groupBy(["one", "two", "three"], "length")
//=> {
//   3: ["one", "two"],
//   5: ["three"],
// }
```

## Related

- [countBy](/docs/Array/countBy)
- [chunk](/docs/Array/chunk)

## Try it

<TryIt code={tryItCode} />
