import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(insertAt([1, 2, 3], 1, 5)); //=> [1, 5, 2, 3]\n\nconsole.log(insertAt([1, 2, 3], 0, 5, 6)); //=> [5, 6, 1, 2, 3]\n\nconsole.log(insertAt([1, 2, 3], 3, 4)); //=> [1, 2, 3, 4]";
export const metadata = {
  title: "insertAt — Array",
  description: "Inserts items at the given index into the array."
};

🎯 Inserts items at the given index into the array.

## Syntax

```ts
import { insertAt } from '@opentf/std';
insertAt<T>(arr: T[], index: number, ...items: T[]): T[]
```

## Parameters

| Name   | Type       | Description                     |
| ------- | ---------- | ------------------------------- |
| arr     | `T[]`      | The source array.                |
| index   | `number`    | The index to insert items at.    |
| items   | `T[]`      | The items to insert.             |

## Returns

A new array with the inserted items.

## Examples

```ts
insertAt([1, 2, 3], 1, 5) //=> [1, 5, 2, 3]

insertAt([1, 2, 3], 0, 5, 6) //=> [5, 6, 1, 2, 3]

insertAt([1, 2, 3], 3, 4) //=> [1, 2, 3, 4]
```

## Related

- [removeAt](/docs/Array/removeAt)
- [replaceAt](/docs/Array/replaceAt)

## Try it

<TryIt code={tryItCode} />
