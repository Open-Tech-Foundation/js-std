import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(swap([], 0, 0)); //=> []\n\nconsole.log(swap([], 0, 1)); //=> []\n\nconsole.log(swap([0], 0, 1)); //=> [undefined, 0]\n\nconsole.log(swap([1, 2, 3], 1, 2)); //=> [1, 3, 2]\n\nconsole.log(swap([1, 2, 3], 1, 5)); //=> [1, undefined, 3, , , 2]\n\nconst arr = [{ a: 1 }, { b: 'a' }, { c: [5] }];\nswap(arr, 0, 2)\n//=> [\n//  {\n//    c: [5],\n//  },\n//  {\n//    b: 'a',\n//  },\n//  {\n//    a: 1,\n//  },\n// ]";
import { Callout } from "@opentf/web-docs";
export const metadata = {
  title: "swap — Array",
  description: "Swaps the positions of two elements in an array."
};


🔄 Swaps the positions of two elements in an array.

<Callout emoji="✅" type="info">
  **Immutable**: This function does not mutate the original array.
</Callout>

## Syntax

```ts
import { swap } from '@opentf/std';
swap<T>(arr: T[], x: number, y: number): T[]
```

## Parameters

- `arr`: The source array.
- `x`: The index of the first element to swap.
- `y`: The index of the second element to swap.

## Returns

A new array with the elements swapped.

## Examples

```ts
swap([], 0, 0) //=> []

swap([], 0, 1) //=> []

swap([0], 0, 1) //=> [undefined, 0]

swap([1, 2, 3], 1, 2) //=> [1, 3, 2]

swap([1, 2, 3], 1, 5) //=> [1, undefined, 3, , , 2]

const arr = [{ a: 1 }, { b: 'a' }, { c: [5] }];
swap(arr, 0, 2)
//=> [
//  {
//    c: [5],
//  },
//  {
//    b: 'a',
//  },
//  {
//    a: 1,
//  },
// ]
```

## Related

- [move](/docs/Array/move)

## Try it

<TryIt code={tryItCode} />
