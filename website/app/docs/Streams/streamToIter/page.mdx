...
export const tryItCode = "import { streamToIter, mapIterAsync, toArrayIterAsync } from '@opentf/std';\nconst sizes = mapIterAsync(streamToIter(response.body), (chunk) => chunk.length);\n\nconsole.log(await toArrayIterAsync(sizes)); //=> [64, 64, 12]";
import { Callout } from "@opentf/web-docs";
export const metadata = {
  title: "streamToIter — Streams",
  description: "Reads a stream as an async iterable, so the *IterAsync operators apply to it."
};
