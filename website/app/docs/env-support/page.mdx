...
- Random and UUID helpers prefer `globalThis.crypto` and fall back to `node:crypto.webcrypto` in Node-compatible environments.
- Hash and HMAC helpers prefer Web Crypto and fall back to `node:crypto` where appropriate.
