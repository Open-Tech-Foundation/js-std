...
| Node.js (default), Bun, LLRT | ❌ | `Intl` |
| Safari | ❌ Technology Preview only | `Intl` |

Browser support is now the majority position — Chrome, Edge and Firefox all ship Temporal, with Safari the remaining holdout. Server runtimes lag: Bun has none at all and Node.js needs a flag, so the `Intl` backend is what most server-side code will run on for some time yet.
