...
_Measured on 2026-09-11 against `@opentf/std@0.19.0`, running the full suite of 1686 tests on each runtime._

| Runtime | Version | Tests passing | Status |
| --- | --- | --- | --- |
| Node.js 20 | `22.23.2` | 1685 / 1686 (99.9%) | ⚠️ Partial |
| Node.js 22 | `22.23.2` | 1685 / 1686 (99.9%) | ⚠️ Partial |
| Node.js 24 | `22.23.2` | 1685 / 1686 (99.9%) | ⚠️ Partial |
| Node.js 26 | `22.23.2` | 1685 / 1686 (99.9%) | ⚠️ Partial |
| Bun | `1.4.2` | 1743 / 1743 (100%) | ✅ Fully supported |
| Deno | `2.9.6` | 1743 / 1743 (100%) | ✅ Fully supported |
| ES-Runtime | `0.28.0` | 1743 / 1743 (100%) | ✅ Fully supported |
| LLRT | `0.9.0-beta` | 1667 / 1743 (95.6%) | ⚠️ Partial |
...
**Runtime spec deviation** (21 tests)

LLRT's Temporal implementation returns invalid zone offsets and disagrees with the current Temporal semantics for fields, transitions and milliseconds. The DateTime fallback is correct; the fix belongs in LLRT.

Affected utilities: `DateTime`, `Duration`. Everything else works as expected.
...
LLRT's JSON implementation does not correctly support revivers, Temporal values and replacer arrays in these cases. The fix belongs in LLRT.

Affected utilities: `tryParseJSON`, `tryStringifyJSON`. Everything else works as expected.
...
**Runtime spec deviation** (9 tests)

LLRT exhausts its JavaScript stack before the library's documented 512-level recursion limit, so the expected limit boundary cannot be reached. The fix belongs in LLRT.

Affected utilities: `clone`, `isEql`, `deepFreeze`, `merge`, `mergeAll`. Everything else works as expected.

**Runtime spec deviation** (3 tests)

LLRT does not preserve own `__proto__` data properties with standard object semantics, so the prototype-safety cases cannot be represented correctly. The fix belongs in LLRT.

Affected utilities: `flattenObject`, `unflattenObject`. Everything else works as expected.
