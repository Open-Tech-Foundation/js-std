import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(truncate('hi-package', 8)); //=> 'hi-pa...'\n\nconsole.log(truncate('hi-package', 5, '---')); //=> 'hi---'\n\nconsole.log(truncate('hi-package', 2)); //=> '..'";
export const metadata = {
  title: "truncate — String",
  description: "Truncates the given string if it's longer than the given maximum string length."
};

Truncates the given string if it's longer than the given maximum string length.

## Syntax

```ts
import { truncate } from '@opentf/std';
truncate(
  str: string,
  length = 30,
  omission = '...'
): string;
```

`length` must be a non-negative finite integer.

The result never exceeds `length`: when `omission` does not fit, it is itself truncated.

## Examples

```ts
truncate('hi-package', 8) //=> 'hi-pa...'

truncate('hi-package', 5, '---') //=> 'hi---'

truncate('hi-package', 2) //=> '..'
```

## Try it

<TryIt code={tryItCode} />
