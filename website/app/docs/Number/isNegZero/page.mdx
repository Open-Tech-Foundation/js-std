import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(isNegZero(0)); //=> false\n\nconsole.log(isNegZero(-0)); //=> true\n\nconsole.log(isNegZero(-0.0)); //=> true\n\nconsole.log(isNegZero(-0x0)); //=> true";
import { Callout } from "@opentf/web-docs";
export const metadata = {
  title: "isNegZero — Number",
  description: "Checks if a number is negative zero (-0)."
};


➖ Checks if a number is negative zero (`-0`).

<Callout type="info">
  In JavaScript, standard equality `0 === -0` returns `true`. Use this function or `Object.is()` to distinguish them.
</Callout>

## Syntax

```ts
import { isNegZero } from '@opentf/std';
isNegZero(n: number): boolean
```

## Parameters

- `n`: The number to check.

## Returns

`true` if the number is `-0`, `false` otherwise.

## Examples

```ts
isNegZero(0) //=> false

isNegZero(-0) //=> true

isNegZero(-0.0) //=> true

isNegZero(-0x0) //=> true
```

## Related

- [isZero](/docs/Number/isZero)

## Try it

<TryIt code={tryItCode} />
