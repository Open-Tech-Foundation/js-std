import TryIt from '../../../components/TryIt.jsx';
export const tryItCode = "console.log(inRange(3, 0, 5)); //=> true\nconsole.log(inRange(-1, 0, 5)); //=> false";
export const metadata = {
  title: "inRange — Number",
  description: "Checks if a number is within the specified range (inclusive)."
};

📏 Checks if a number is within the specified range (inclusive).

## Syntax

```ts
import { inRange } from '@opentf/std';
inRange(n: number, start: number, end: number): boolean;
```

## Parameters

| Name  | Type     | Description              |
| ----- | -------- | ------------------------ |
| n     | `number` | The number to check.     |
| start | `number` | The start of the range.  |
| end   | `number` | The end of the range.    |

## Returns

True if n is between start and end.

## Examples

```ts
inRange(3, 0, 5) //=> true
inRange(-1, 0, 5) //=> false
```

## Try it

<TryIt code={tryItCode} />
