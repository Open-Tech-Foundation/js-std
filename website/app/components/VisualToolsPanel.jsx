...
// styles via VisualTools.css in global.css
      ...
      <AnimatePresence>
        {events.filter(e => now - e.time < duration).map(event => (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className={cn("eventDot", event.type === 'request' ? "eventDotRequest" : "eventDotExecution")}
            style={{ 
      ...
      <div className={"card"}>
        ...
        <div className={"controls"}>
          <div className={"controlGroup"}>
            <label className={"label"}>Delay (ms): {delay}</label>
            <input className={"inputRange"} type="range" min="100" max="2000" step="100" value={delay} onChange={e => setDelay(Number(e.target.value))} />
          </div>
          <div className={"controlGroup"}>
            <label className={"label"}>Max Wait (ms): {maxWait === 0 ? 'None' : maxWait}</label>
            <input className={"inputRange"} type="range" min="0" max="5000" step="500" value={maxWait} onChange={e => setMaxWait(Number(e.target.value))} />
          </div>
          <label className="flex items-center gap-2 cursor-pointer text-sm">
            <input type="checkbox" checked={leading} onChange={e => setLeading(e.target.checked)} /> Leading Edge
          </label>
          <button className={cn("btn", "btnPrimary")} onClick={() => {
            addEvent('request');
            debouncedRef.current();
          }}>
            <Play size={18} /> Call Function
          </button>
          <button className={"btn"} style={{background: 'rgba(128,128,128,0.1)'}} onClick={() => setEvents([])}>
  ...
  return (
    <div className={"grid2"}>
      <div className={"card"}>
        ...
        <div className={"controls"}>
          <div className={"controlGroup"}>
            <label className={"label"}>Interval (ms): {interval}</label>
            <input className={"inputRange"} type="range" min="100" max="2000" step="100" value={interval} onChange={e => setIntervalTime(Number(e.target.value))} />
          </div>
          ...
          <button className={cn("btn", "btnPrimary")} style={{background: '#8b5cf6'}} onClick={() => {
            addEvent('request');
  ...
  return (
    <div className={"grid2"}>
      <div className={"card"}>
        ...
        <div className={"controls"}>
          <div className={"controlGroup"}>
            <label className={"label"}>Limit (Items): {limit}</label>
            <input className={"inputRange"} type="range" min="1" max="20" value={limit} onChange={e => setLimit(Number(e.target.value))} />
          </div>
          <div className={"controlGroup"}>
            <label className={"label"}>Wait Delay (ms): {delay}</label>
            <input className={"inputRange"} type="range" min="0" max="5000" step="500" value={delay} onChange={e => setDelay(Number(e.target.value))} />
          </div>
          <button className={cn("btn", "btnPrimary")} style={{background: '#10b981'}} onClick={triggerCall}>
      ...
      <div className="flex flex-col gap-4">
        <div className={"card"} style={{padding: '1.25rem'}}>
          <div className={"statLabel"}>Pending Batch ({pendingItems.length}/{limit})</div>
          <div className="flex flex-wrap gap-2 mt-2 min-h-[40px]">
            {pendingItems.map((item, i) => (
              <motion.div 
                key={i} 
                initial={{scale: 0}} animate={{scale: 1}}
                className={cn("badge", "badgeBlue", "px-3 py-2")}
              >
                {item}
              </motion.div>
            ))}
        ...
        <div className={"card"} style={{padding: '1.25rem'}}>
          <div className={"statLabel"}>Processed Batches</div>
  ...
  return (
    <div className={"grid2"}>
      <div className={"card"}>
        ...
        <div className={"controls"}>
          <div className={"controlGroup"}>
            <label className={"label"}>Limit (Calls): {limit}</label>
            <input className={"inputRange"} type="range" min="1" max="10" value={limit} onChange={e => setLimit(Number(e.target.value))} />
          </div>
          <div className={"controlGroup"}>
            <label className={"label"}>Period (ms): {period}</label>
            <input className={"inputRange"} type="range" min="500" max="5000" step="500" value={period} onChange={e => setPeriod(Number(e.target.value))} />
          </div>
          <button className={cn("btn", "btnPrimary")} style={{background: '#f59e0b'}} onClick={triggerCall}>
  ...
  return (
    <div className={"grid2"}>
      <div className={"card"}>
        ...
        <div className={"controls"}>
          <div className={"controlGroup"}>
            <label className={"label"}>Max Retries: {retries}</label>
            <input className={"inputRange"} type="range" min="1" max="10" value={retries} onChange={e => setRetries(Number(e.target.value))} />
          </div>
          <div className={"controlGroup"}>
            <label className={"label"}>Delay (ms): {delay}</label>
            <input className={"inputRange"} type="range" min="0" max="2000" step="100" value={delay} onChange={e => setDelay(Number(e.target.value))} />
          </div>
          <div className={"controlGroup"}>
          ...
          <button className={cn("btn", "btnPrimary")} onClick={runTest} disabled={status === 'running'}>
      ...
      <div className="flex flex-col gap-4">
        <div className={"card"} style={{padding: '1.25rem'}}>
          <div className={"statLabel"}>Execution Status: {status}</div>
    ...
    <div className={"root"}>
      <div className={"toolLayout"}>
        <aside className={"toolSidebar"}>
          <div className={cn("navItem", activeTab === 'idle' && "navItemActive")} onClick={() => setActiveTab('idle')}>
            <Clock size={18} /> idleRun
          </div>
          <div className={cn("navItem", activeTab === 'pace' && "navItemActive")} onClick={() => setActiveTab('pace')}>
            <Activity size={18} /> paceRun
          </div>
          <div className={cn("navItem", activeTab === 'batch' && "navItemActive")} onClick={() => setActiveTab('batch')}>
            <Layers size={18} /> batchRun
          </div>
          <div className={cn("navItem", activeTab === 'rate' && "navItemActive")} onClick={() => setActiveTab('rate')}>
            <Cpu size={18} /> rateLimitRun
          </div>
          <div className={cn("navItem", activeTab === 'retry' && "navItemActive")} onClick={() => setActiveTab('retry')}>
