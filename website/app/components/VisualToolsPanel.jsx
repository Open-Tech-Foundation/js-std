...
const IdleRunTab = () => {
  ...
  const addEvent = useCallback((type) => {
    setEvents((prev) => [
      ...prev,
      { id: Math.random().toString(36), type, time: Date.now() },
    ]);
  }, []);
  ...
  useEffect(() => {
    debouncedRef.current = idleRun(
      ...
      delay,
      { leading, maxWait: maxWait > 0 ? maxWait : undefined },
    );
        ...
        <div className={'controls'}>
          ...
          <div className={'controlGroup'}>
            <label className={'label'}>
              Max Wait (ms): {maxWait === 0 ? 'None' : maxWait}
            </label>
            <input
              className={'inputRange'}
              type="range"
      ...
      <div className="flex flex-col gap-4">
        <div className={'card'} style={{ padding: '1.25rem' }}>
          <div className={'statLabel'}>
            Pending Batch ({pendingItems.length}/{limit})
          </div>
...
const RateLimitRunTab = () => {
  ...
  const addEvent = useCallback((type, label) => {
    setEvents((prev) => [
      ...prev,
      { id: Math.random().toString(36), type, time: Date.now(), label },
    ]);
  }, []);

  useEffect(() => {
    rateLimitedRef.current = rateLimitRun(
      async (id) => {
    ...
    try {
      await retryRun(
        async () => {
          ...
          setLogs((prev) => [
            `Attempt ${attempts}: ${isLastAttempt ? 'Success!' : 'Failed, retrying...'}`,
            ...prev,
          ]);
...
const tabs = [
  { id: 'idle', label: 'idleRun', Icon: Clock },
  { id: 'pace', label: 'paceRun', Icon: Activity },
  { id: 'batch', label: 'batchRun', Icon: Layers },
  { id: 'rate', label: 'rateLimitRun', Icon: Cpu },
  { id: 'retry', label: 'retryRun', Icon: RefreshCw },
];
        ...
        <aside className={'toolSidebar'}>
          {tabs.map(({ id, label, Icon }) => (
            <button
              key={id}
              type="button"
              className={cn('navItem', activeTab === id && 'navItemActive')}
