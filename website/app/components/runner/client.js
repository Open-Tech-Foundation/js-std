 ...
 * @param {{ prelude?: string, timeout?: number, onEvent: Function,
 *   onProbes?: Function }} options `onProbes` receives the probe table before
 *   the run starts, so a caller can label a result with the expression that
 *   produced it.
 * @returns {Promise<void>} Settles when the run finishes, times out, or fails.
 */
export default function execute(source, options) {
  const { prelude = '', timeout = DEFAULT_TIMEOUT, onEvent } = options;
